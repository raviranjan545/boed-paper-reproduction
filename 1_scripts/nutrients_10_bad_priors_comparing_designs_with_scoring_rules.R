...
here::i_am("1_scripts/nutrients_10_bad_priors_comparing_designs_with_scoring_rules.R")
...
score_one_sim <- function(csv_path,
    ...
    fixed_sds <- c(global_sd_mumax, global_sd_k)
    ...
    crps_k   <- crps_sample(y = true_vals[2], dat = post_samples[, "k"])
    
    c(ES = es_joint, CRPS_mumax = crps_mu, CRPS_k = crps_k)
}
...
base_dir <- here("nutrient_curves_bad_priors_posteriors")
...
for (i in 1:length(n_points_list)){
    ...
    fit_opt_list <- list.files(
        base_dir, 
        pattern = paste0("^monod_badprior_posterior_opt_", n_points, ".*\\.csv$"), 
        full.names = TRUE
    )
    fit_unif_list <- list.files(
        base_dir, 
        pattern = paste0("^monod_badprior_posterior_unif_", n_points, ".*\\.csv$"), 
        full.names = TRUE
    )
    ...
    for (j in 1:n_sims) {
        ...
        true_vals <- c(params$mu_max[j], params$k[j])
