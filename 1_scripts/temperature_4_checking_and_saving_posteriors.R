...
here::i_am("1_scripts/temperature_4_checking_and_saving_posteriors.R")
...
for (i in 1:length(n_points_list)){
    ...
    fit_opt_list <- list.files(here("temperature_curves_simulations_and_fits"), 
                               pattern = paste0("^norberg_fit_opt_", n_points), full.names = TRUE)
    fit_unif_list <- list.files(here("temperature_curves_simulations_and_fits"), 
                                pattern = paste0("^norberg_fit_unif_", n_points), full.names = TRUE)
    ...
    for (j in 1:length(fit_unif_list)) {
        ...
        draws_opt <- as_tibble(fit_opt) %>%
            select(c("b_loga_Intercept", "b_logb_Intercept", "b_tmax_Intercept", "b_tmin_Intercept")) %>%
            mutate(                                  
                b_loga_Intercept = b_loga_Intercept,   
                b_logb_Intercept = b_logb_Intercept,
                loga_corrected = b_loga_Intercept + (exp(b_logb_Intercept) * 20.88513513513514),
                b_tmax_Intercept = b_tmax_Intercept,
                b_tmin_Intercept = b_tmin_Intercept
            )
        ...
        write.csv(draws_opt, 
                  here("temperature_curves_posteriors/", 
                       paste0(sprintf("norberg_posterior_opt_%spts_%04d", n_points, j), 
                              ".csv")), 
                  row.names = FALSE)
        write.csv(draws_unif, 
                  here("temperature_curves_posteriors/", 
                       paste0(sprintf("norberg_posterior_unif_%spts_%04d", n_points, j), 
                              ".csv")), 
                  row.names = FALSE)
...
if(nrow(fits_to_examine) > 0){
    write.csv(fits_to_examine,
              here("3_simulation_result_summaries",
                   paste0("norberg_problem_fits_to_check_", 
                          n_points, 
                          ".csv")
              ), 
              row.names = FALSE)
