...
here::i_am("1_scripts/light_7_visualising_design_comparison.R")
...
# Uniform
unif_light5_levels <- round(seq(10, 1000, length.out = 5), -1)
unif_light7_levels <- round(seq(10, 1000, length.out = 7), -1)
unif_light10_levels <- round(seq(10, 1000, length.out = 10), -1)
unif_light15_levels <- round(seq(10, 1000, length.out = 15), -1)

# Joining optimal and uniform designs into one data frame
designs <- data.frame(levels = c(opt_light5_levels, opt_light7_levels, 
                                 opt_light10_levels, opt_light15_levels,
                                 unif_light5_levels, unif_light7_levels, 
                                 unif_light10_levels, unif_light15_levels)) %>%
...
rm(opt_light5_levels, opt_light7_levels, opt_light10_levels, opt_light15_levels,
   unif_light5_levels, unif_light7_levels, unif_light10_levels, unif_light15_levels)

## CRPS and ES scores
all_scores <- read.csv(here("3_simulation_result_summaries", 
                            "eilerspeeters_fit_opt_unif_scoring_rules_comparison.csv")) %>%
...
pct_opt_better <- all_scores %>%
    select(n_points, simulation, design, CRPS_mumax, CRPS_alpha, CRPS_Iopt, ES) %>%
    pivot_wider(
        names_from = design,
        values_from = c(CRPS_mumax, CRPS_alpha, CRPS_Iopt, ES)
    ) %>%
    group_by(n_points) %>%
    summarise(
        mumax_opt_better = 100 * mean(CRPS_mumax_opt < CRPS_mumax_unif, na.rm = TRUE),
        alpha_opt_better = 100 * mean(CRPS_alpha_opt < CRPS_alpha_unif, na.rm = TRUE),
        Iopt_opt_better = 100 * mean(CRPS_Iopt_opt < CRPS_Iopt_unif, na.rm = TRUE),
        ES_opt_better = 100 * mean(ES_opt < ES_unif, na.rm = TRUE),
        .groups = "drop"
    ) %>%
    mutate(
        label_mumax = paste0(round(mumax_opt_better), "%"),
        label_alpha = paste0(round(alpha_opt_better), "%"),
        label_Iopt = paste0(round(Iopt_opt_better), "%"),
...
dat_mbd_curve <- tibble(
    ...
    alpha = central_curve_params$alpha,
    i_opt = central_curve_params$i_opt
) %>%
    mutate(y = purrr::pmap_dbl(list(driver, mu_max, alpha, i_opt), 
                               eilers_peeters))
...
a1 <- ggplot(dat_plot, aes(x = driver, y = y, group = curve_id)) +
    ...
    labs(
        title = "a) Light curves",
        x = expression(Light~(mu*E~m^{-2}~s^{-1})),
        y = expression(Growth~rate~(day^{-1}))
    ) +
    scale_fill_manual(values = cols, name = NULL,
                      labels = c("Optimal", "Uniform")) +
    geom_rect(data = dummy, aes(fill = Design), inherit.aes = FALSE,
              xmin = Inf, xmax = Inf, ymin = Inf, ymax = Inf) +   # invisible; just makes box keys
...
p_alpha <- ridgeline_diff(all_scores, 
                          CRPS_alpha, 
                          label_x = 0.16,
                          label_y_offset = 0.7,
                          labels = pct_opt_better,
                          label_col = "label_alpha",
                          title = expression("d)"~Estimation~score~`difference,`~alpha)) + 
    labs(x = expression(CRPS["opt"] - CRPS["unif"]~"("*day^{-1}~(mu*E~m^{-2}~s^{-1}*")"^{-1})),
         y = "")

p_iopt <- ridgeline_diff(all_scores, 
                         CRPS_Iopt, 
                         label_x = 0.16,
                         label_y_offset = 0.7,
                         labels = pct_opt_better,
                         label_col = "label_Iopt",
                         title = expression("e)"~Estimation~score~`difference,`~italic(I)[opt])) + 
    labs(x = expression(CRPS["opt"] - CRPS["unif"]~"("*mu*E^{-1}~m^{2}~s^{-1}*")"))
...
t <- ggdistribution(dlnorm, 
                    ...
                    colour = NA,
                    ylab = "",
                    xlab = expression(alpha~"("*day^{-1}~(mu*E~m^{-2}~s^{-1}*")"^{-1}))) + 
    theme(
        panel.grid.minor = element_blank()
    ) + 
    ggtitle(expression("b)"~alpha))
...
u <- ggdistribution(dlnorm, 
                    ...
                    colour = NA,
                    ylab = "Probability density",
                    xlab = expression(italic(I)[opt]~"("*mu*E^{-1}~m^{2}~s^{-1}*")")) + 
    theme(
        panel.grid.minor = element_blank()
    ) + 
    ggtitle(expression("c)"~italic(I)[opt]))
...
cols_to_mean <- c("CRPS_mumax", "CRPS_alpha", "CRPS_Iopt", "ES")
...
# B. Alpha
s2 <- all_scores %>%
    ggplot(., aes(CRPS_alpha, factor(n_points), group = interaction(design, n_points), 
                  fill = design)) + 
    geom_violin(quantile.linetype = 1,
                quantiles = 0.5) + 
    geom_point(
        data = mean_score,
        aes(x = mean_CRPS_alpha, y = factor(n_points)),
        size = 4, colour = "red",
        position = position_dodge2(width = 0.9)
    ) +
    scale_fill_manual(values = c("orange", "purple")) + 
    ...
    scale_x_log10() + 
    ylab("") + 
    xlab(expression(CRPS~"("*day^{-1}~(mu*E~m^{-2}~s^{-1}*")"^{-1}))) +  
    ggtitle(expression(paste("b) Estimation score, ", alpha)),
            subtitle = "lower is better") + 
    guides(fill = "none") + 
    theme( 
        plot.title = element_text(size = 15),
...
# C. Iopt
s3 <- all_scores %>%
    ggplot(., aes(CRPS_Iopt, factor(n_points), group = interaction(design, n_points), 
                  fill = design)) + 
    geom_violin(quantile.linetype = 1,
                quantiles = 0.5) + 
    geom_point(
        data = mean_score,
        aes(x = mean_CRPS_Iopt, y = factor(n_points)),
        size = 4, colour = "red",
        position = position_dodge2(width = 0.9)
    ) +
    scale_fill_manual(values = c("orange", "purple")) + 
    ...
    scale_x_log10() + 
    ylab("# of experimental units") + 
    xlab(expression(CRPS~"("*mu*E^{-1}~m^{2}~s^{-1}*")")) + 
    ggtitle(expression(paste("c) Estimation score, ", italic(I)[opt])),
            subtitle = "lower is better") + 
    guides(fill = "none") + 
    theme( 
        plot.title = element_text(size = 15),
...
driver_values <- seq(0, 1000, 5)

# Plot curves from simulated parameter combinations
dat_plot <- params %>%
    mutate(driver = list(driver_values)) %>%
    tidyr::unnest(driver) %>%
    mutate(y = purrr::pmap_dbl(list(driver, mu_max, alpha, i_opt), eilers_peeters))
...
ggplot(dat_plot, aes(x = driver, y = y, group = mu_max)) +
    ...
    labs(
        title = "Light curves simulated from priors",
        x = expression(Light~(mu*E~m^{-2}~s^{-1})),
        y = expression(Growth~rate~(day^{-1}))
    ) +
    theme(
...
ggsave(
    here(
        "4_figures",
        "light_SI_1000_simulated_curves.pdf"),
