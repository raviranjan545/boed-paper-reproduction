...
here::i_am("1_scripts/light_4_checking_and_saving_posteriors.R")
...
alpha_adjustment <- exp(-3)
i_opt_adjustment <- exp(5.5)
...
for (i in 1:length(n_points_list)){
    ...
    fit_opt_list <- list.files(here("light_curves_simulations_and_fits"), 
                               pattern = paste0("^eilerspeeters_fit_opt_", n_points), 
                               full.names = TRUE)
    fit_unif_list <- list.files(here("light_curves_simulations_and_fits"), 
                                pattern = paste0("^eilerspeeters_fit_unif_", n_points), 
                                full.names = TRUE)
    ...
    for (j in 1:length(fit_unif_list)) {
        ...
        draws_opt <- as_tibble(fit_opt) %>%
            select(c("b_mumax_Intercept", "b_alpha_Intercept", "b_iopt_Intercept")) %>%
            mutate(b_mumax_Intercept = b_mumax_Intercept * mu_max_adjustment,
                   b_alpha_Intercept = b_alpha_Intercept * alpha_adjustment,
                   b_iopt_Intercept = b_iopt_Intercept * i_opt_adjustment
            )
        ...
        write.csv(draws_opt, 
                  here("light_curves_posteriors/", 
                       paste0(sprintf("eilerspeeters_posterior_opt_%spts_%04d", n_points, j), 
                              ".csv")), 
                  row.names = FALSE)
        write.csv(draws_unif, 
                  here("light_curves_posteriors/", 
                       paste0(sprintf("eilerspeeters_posterior_unif_%spts_%04d", n_points, j), 
                              ".csv")), 
                  row.names = FALSE)
...
if(nrow(fits_to_examine) > 0){
    write.csv(fits_to_examine,
              here("3_simulation_result_summaries",
                   paste0("eilerspeeters_problem_fits_to_check_", 
                          n_points, 
                          ".csv")
              ), 
              row.names = FALSE)
