...
here::i_am("1_scripts/nutrients_3_simulation_and_bayesian_fitting.R")
...
nutrient_folder <- "nutrient_curves_simulations_and_fits"
...
for (i in 1:length(n_points_list)){
    ...
    for (j in 1:n_sims){
        ...
        opt_base <- sprintf("monod_fit_opt_%spts_%04d", n_points, j)
        unif_base <- sprintf("monod_fit_unif_%spts_%04d", n_points, j)
        ...
        fit_opt <- update(
            ...
            cores = 4, 
            seed = 1785,
            file = here(nutrient_folder, paste0(opt_base, ".rds")),
            file_refit = "always",
            refresh = 0,
            control = list(
                adapt_delta = 0.95,
        ...
        fit_unif <- update(
            ...
            cores = 4, 
            seed = 1785,
            file = here(nutrient_folder, paste0(unif_base, ".rds")),
            file_refit = "always",
            refresh = 0,
            control = list(
                adapt_delta = 0.95,
