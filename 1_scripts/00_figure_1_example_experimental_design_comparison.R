...
here::i_am("1_scripts/00_figure_1_example_experimental_design_comparison.R")
...
p3 <- ggplot(sim_results, aes(x = mu_est, fill = Design)) +
    ...
    geom_vline(xintercept = mu_mean_opt, color = cols["Custom"], linetype = "solid", linewidth = 1) +  
    scale_fill_manual(values = cols) +
...
p4 <- ggplot(sim_results, aes(x = K_est, fill = Design)) +
    ...
    geom_vline(xintercept = K_mean_opt, color = cols["Custom"], linetype = "solid", linewidth = 1) +  
    scale_fill_manual(values = cols) +
...
ggsave(here(
    "4_figures",
    "figure_1_monod_custom_vs_uniform_design_example.pdf"), 
    plot = final_plot, width = 10*2.54, height = 8*2.54, units = "cm"
)
