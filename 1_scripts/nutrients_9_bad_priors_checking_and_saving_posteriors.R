...
here::i_am("1_scripts/nutrients_9_bad_priors_checking_and_saving_posteriors.R")
...
for (i in 1:length(n_points_list)){
    ...
    fit_opt_list <- list.files(here("nutrient_curves_bad_priors_simulations_and_fits"), 
                               pattern = paste0("^monod_badprior_fit_opt_", n_points), full.names = TRUE)
    fit_unif_list <- list.files(here("nutrient_curves_bad_priors_simulations_and_fits"), 
                                pattern = paste0("^monod_badprior_fit_unif_", n_points), full.names = TRUE)
    ...
    for (j in 1:length(fit_unif_list)) {
        ...
        write.csv(draws_opt, 
                  here("nutrient_curves_bad_priors_posteriors/", 
                       paste0(sprintf("monod_badprior_posterior_opt_%spts_%04d", n_points, j), 
                              ".csv")), 
                  row.names = FALSE)
        write.csv(draws_unif, 
                  here("nutrient_curves_bad_priors_posteriors/", 
                       paste0(sprintf("monod_badprior_posterior_unif_%spts_%04d", n_points, j), 
                              ".csv")), 
                  row.names = FALSE)
...
if(nrow(fits_to_examine) > 0){
    write.csv(fits_to_examine,
              here("3_simulation_result_summaries",
                   paste0("monod_badprior_problem_fits_to_check_", 
                          n_points, 
                          ".csv")
              ), 
              row.names = FALSE)
