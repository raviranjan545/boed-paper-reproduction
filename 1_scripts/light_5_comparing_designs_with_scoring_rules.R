...
here::i_am("1_scripts/light_5_comparing_designs_with_scoring_rules.R")
...
global_sd_alpha  <- sd_lognormal(meanlog = -3, sdlog = 0.8)
global_sd_iopt  <- sd_lognormal(meanlog = 5.5, sdlog = 0.3)


# Define function to score a single simulation (one RDS path + one row of truth)
score_one_sim <- function(csv_path,
                          true_vals,  
                          cols = c("b_mumax_Intercept", "b_alpha_Intercept", "b_iopt_Intercept")) {
    ...
    fixed_sds <- c(global_sd_mumax, global_sd_alpha, global_sd_iopt)
    ...
    crps_alpha   <- crps_sample(y = true_vals[2], dat = post_samples[, "alpha"])
    crps_iopt   <- crps_sample(y = true_vals[3], dat = post_samples[, "I_opt"])
    
    c(ES = es_joint, CRPS_mumax = crps_mu, CRPS_alpha = crps_alpha, CRPS_Iopt = crps_iopt)
}
...
for (i in 1:length(n_points_list)){
    ...
    for (j in 1:n_sims) {
        ...
        print(c(i, j))
        true_vals <- c(params$mu_max[j], params$alpha[j], params$i_opt[j])
