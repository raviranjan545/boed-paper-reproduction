...
[Ranjan, R., & Thomas, M. K. (2026). Bayesian optimal designs for common single-driver experiments in ecology. *bioRxiv*, 2026-05.](https://www.biorxiv.org/content/10.64898/2026.05.28.728579v3.abstract)
...
- Light (Eilers-Peeters model)

- Nutrients (Monod model)

- Temperature (Norberg model)

- Toxins (log-logistic model)

The majority of the code here relates to *evaluating* the optimal designs using simulations, which takes time and hundreds of gigabytes of drive space. If you are interested mainly in *generating* optimal designs yourself, we provide an alternate [repository](https://github.com/raviranjan545/boed-calculation) with a minimal number of simpler scripts that omits the simulation component. Note that generating optimal designs can also take multiple days if calculated at high precision. Therefore, by default, the code in the alternate [repository](https://github.com/raviranjan545/boed-calculation) generates designs with just 5 experimental units and uses lower precision than the code here. But those scripts are designed to be easy to modify; you are encouraged to change different settings like priors and design space and investigate their impact on the resulting designs.
